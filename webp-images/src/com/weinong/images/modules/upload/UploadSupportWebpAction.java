 ...
 */
public class UploadSupportWebpAction extends BaseApiAction {
    ...
    public ApiResult doApi() throws Exception {
        ...
        String[] thumb_source_files = createFiles(basePath, "original", ".jpg", 0, thumbnails.size());//每个thumbnail生成webp所需要的原图
...
//            String thumbnail_source = thumb_source_files[times];
    ...
    private String[] createTargetFiles(String basePath, String whole_group, String fileSuff, int startmark, int endmark, String originalWH) {
        ...
        for (; startmark < endmark; startmark++) {
            ...
            String path = basePath + meta.getMd5ForFile() + "_" + whole_group + "_" + startmark + "_" + sizeInfo + "_" + format + fileSuff;
    ...
    private void createBothImagesDependThumbnail(List<String> imagefilesforWater, String originalFile, String[] thumb_source_files, String[] thumb_jpg_files, String[] thumb_webp_files, String imageSize) throws Exception {
        ...
        for (Thumbnail oneThumbnail : thumbnails) {// 判断要生成几张缩略图，
            ...
            if (oneThumbnail.getSize().equals("normal")) { // 因为size参数很重要，如果size参数没有的话，那么type参数就不起作用了，只进行压缩//
                ...
                ImageUtil2.compressImage(originalFile, thumbnail_source, 1.0f);

                ImageUtil2.compressImage(thumbnail_source, targetPathJpg, oneThumbnail.getQuality() / 100.0f);
                creatWebpImage(thumbnail_source, targetPathWebp, oneThumbnail.getQuality());
    ...
    private void creatWebpImage(String srcpath, String despath, int quality) throws IOException {
        ...
        String mander = "cwebp -q " + quality + " " + srcpath + " -o " + despath;
        Console.info(this, "调用图片生成的命令是 ：" + mander);
